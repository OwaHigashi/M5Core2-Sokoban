...
void fillImage(void *image, int x, int y, int w, int h) {
  ...
  M5.Lcd.pushImage(x, y, w, h, (uint16_t*)image);
...
void flickDetect() {
  if (M5.Touch.points > 0 && M5.Touch.point[0].y < 240) {
    ...
    if (absx > FLICK_THRESHOLD || absy > FLICK_THRESHOLD) {
      if (absx > absy) {
        flick_dir = (dx > 0) ? RIGHT : LEFT;
      } else {
        flick_dir = (dy > 0) ? DOWN : UP;
      }
      flick_detected = true;
    }
  }
...
bool SerialAction(Dir* dir, bool* restart) {
  ...
  // Serial controller still available
