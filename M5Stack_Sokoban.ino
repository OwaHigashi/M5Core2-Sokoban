...
void fillImage(void *image, int x, int y, int w, int h) {
  ...
  // pushImageを使えば、アドレスウィンドウ指定やループ処理は不要。
  M5.Lcd.pushImage(x, y, w, h, p);
}
...
bool SerialAction(Dir* dir, bool* restart) {
  ...
  if(M5.BtnA.wasPressed()) {
    ...
    *dir = RIGHT;
    ...
    atTP = M5.Touch.getPressPoint();
    Serial.print(atTP.x);
    Serial.print(",");
    Serial.println(atTP.y);
    if((atTP.x > 50) && (atTP.x < M5.Lcd.width()-50) && (atTP.y > M5.Lcd.height()-150) && (atTP.y < M5.Lcd.height()-20) ){
      delay(300);
      *dir = UP;
      return true;
    } else if((atTP.x > 50) && (atTP.x < M5.Lcd.width()-50) && (atTP.y > 20) && (atTP.y < 100) ){
      delay(300);
      *restart = true;
      return false;
    }
